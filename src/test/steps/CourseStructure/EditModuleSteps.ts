...
When('the Admin clicks the Edit option', async function () {
    ...
    await this.courseStructurePage.editModule(editData.ExistingTitle);
});
...
When('the Admin updates the module with valid data from CSV', async function () {
    ...
    const skills = editData.UpdatedSkills.split(",").map((skill: string) => skill.trim());
    await this.courseStructurePage.updateModule(editData.UpdatedTitle,editData.UpdatedDescription,skills);
});
